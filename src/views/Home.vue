				...
				<pane min-size="20">
					<div class="to" v-loading="loading" element-loading-text="拼命加载中..." element-loading-background="rgba(0,0,0,0)">
						<div class="trans_wrap">
							<div class="socll">
								<template v-if="trans_result.result && trans_result.result.length > 0">
								...
								<!-- <template v-if="trans_result.dict && trans_result.dict.length > 0">
									<div class="item" v-for="(item, idx) in trans_result.dict" :key="idx">
										<div class="src">{{ item.word }}</div>
										<el-check-tag checked title="点击复制" @click="ClickTransItemHandle">{{ item.trans }}</el-check-tag>
									</div>
								</template> -->
								<div v-if="trans_result.dict && trans_result.dict.length > 0" class="rec">
									<div>
										<span class="more">更多</span>
										<table>
											<tr v-for="item in trans_result.dict" :key="item">
												<td>
